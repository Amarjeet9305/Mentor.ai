...
export default async function DashboardPage() {
    ...
    const clerkName = `${clerkUser.firstName || ''} ${clerkUser.lastName || ''}`.trim() || email.split('@')[0];
    ...
    if (!user) {
        ...
        // Sync name and image if they've changed or were placeholders
        const hasPlaceholderName = user.name === "New User" || !user.name;
        const nameChanged = clerkName && user.name !== clerkName;
        const imageChanged = clerkUser.imageUrl && user.image !== clerkUser.imageUrl;

        if (hasPlaceholderName || nameChanged || imageChanged) {
            user.name = clerkName || user.name;
            user.image = clerkUser.imageUrl || user.image;
            await user.save();
            console.log("Synced user data from Clerk in Dashboard");
        }
    }
                                ...
                                <div className="bg-white rounded-2xl border border-slate-200 p-8 shadow-sm">
                                    ...
                                    <div className="bg-slate-50 p-6 rounded-xl border border-slate-100 text-center py-12">
                                        {assessmentResult ? (
                                            ...
                                            <>
                                                <p className="text-slate-500 mb-4">You haven't taken the assessment yet.</p>
                                                <Link href="/assessment">
